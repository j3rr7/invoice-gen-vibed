...
def styled_field(label: str, value: str = "", hint: str = "", icon: str = None,
                 multiline=False, min_lines=1, max_lines=1, width=None,
                 disabled=False, expand=False, on_change=None) -> ft.TextField:
...
def accent_card(content: ft.Control, accent_color: str = "#1e88e5") -> ft.Container:
    ...
    return ft.Container(
        content=ft.Row(
            [
                ft.Container(
                    width=4, bgcolor=accent_color,
                    border_radius=_br_only(tl=12, bl=12)
                ),
...
def main(page: ft.Page):
    # ── Page Config ──
    page.title = "Catering Fajar — Invoice Designer"
    ...
    # ── INPUT FIELDS ──
    comp_name_input   = styled_field("Company Name",   defaults.get("company_name", ""),    icon=ft.Icons.BUSINESS)
    comp_addr_input   = styled_field("Address",        defaults.get("company_address", ""), icon=ft.Icons.LOCATION_ON)
    comp_email_input  = styled_field("Email",          defaults.get("company_email", ""),   icon=ft.Icons.EMAIL)
    comp_phone_input  = styled_field("Phone",          defaults.get("company_phone", ""),   icon=ft.Icons.PHONE)
    comp_web_input    = styled_field("Website",        defaults.get("company_website", ""), icon=ft.Icons.LANGUAGE)
    comp_footer_input = styled_field("Footer Note",    defaults.get("footer_note", ""),     icon=ft.Icons.STICKY_NOTE_2)

    recipient_name_input = styled_field("Client Name / Company *", hint="Required", icon=ft.Icons.PERSON)
    recipient_addr_input = styled_field("Client Address (Optional)", icon=ft.Icons.HOME,
                                        multiline=True, min_lines=2, max_lines=4)

    invoice_title_input  = styled_field("Invoice Title", "Invoice",                        icon=ft.Icons.RECEIPT_LONG)
    invoice_date_input   = styled_field("Invoice Date (DD-MM-YYYY)",
                                        format_date(datetime.datetime.now()),               icon=ft.Icons.CALENDAR_TODAY)
    billing_period_input = styled_field("Billing Period", icon=ft.Icons.DATE_RANGE)

    auto_no_switch   = ft.Switch(value=True, active_color=ACCENT, label="",
                                 inactive_track_color=DIVIDER, width=44)
    ...
    item_desc_input  = styled_field("Item Description", hint="e.g. Catering Makan Siang", expand=True)
    item_qty_input   = styled_field("Qty",        "0",     width=72)
    item_price_input = styled_field("Price (IDR)", "20000", width=130)
    item_date_input  = styled_field("Item Date", format_date(datetime.datetime.now()),
                                    width=140, disabled=True)
    item_date_checkbox = ft.Checkbox(value=False, fill_color=ACCENT, check_color="white",
                                     label="Include Date",
                                     label_style=ft.TextStyle(color=TEXT_SECONDARY, size=12))
    ...
    # ── TOTALS DISPLAY ──
    subtotal_lbl  = ft.Text("IDR 0,00", size=15, weight=ft.FontWeight.BOLD, color=TEXT_PRIMARY)
    tax_title_lbl = ft.Text("Tax (0.5%):", size=13, color=TEXT_SECONDARY)
    tax_val_lbl   = ft.Text("IDR (0,00)", size=13, color=TEXT_SECONDARY)
    total_due_lbl = ft.Text("IDR 0,00",   size=22, weight=ft.FontWeight.BOLD, color=SUCCESS)

    # ── ITEMS LISTVIEW ──
    items_list_view  = ft.ListView(expand=True, spacing=8,
                                   padding=_psym(horizontal=4, vertical=4))
    ...
    def trigger_billing_period_auto_calc():
        ...
        if dt:
            billing_period_input.value = f"{INDONESIAN_MONTHS.get(dt.month, dt.strftime('%B'))} {dt.year}"
    ...
    def trigger_invoice_auto_gen():
        if not auto_no_switch.value:
            return
        dt = parse_date(invoice_date_input.value)
        date_token = dt.strftime("%Y%m") if dt else datetime.datetime.now().strftime("%Y%m")
        initials = get_client_initials(recipient_name_input.value)
        try:
            counter = int(counter_input.value.strip()) if counter_input.value.strip() else 1
        except ValueError:
            counter = 1
    ...
    invoice_date_input.on_change   = lambda e: (trigger_billing_period_auto_calc(), trigger_invoice_auto_gen())
        ...
        if not items_list:
            items_list_view.controls.append(
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Icon(ft.Icons.RECEIPT_LONG_OUTLINED, color=DIVIDER, size=40),
                            ft.Text("No items added yet", color=TEXT_SECONDARY, size=13),
                            ft.Text("Use the form above to add catering items.", color=DIVIDER, size=11)
                        ],
            ...
            for idx, item in enumerate(items_list):
                ...
                card = ft.Container(
                    content=ft.Row(
                        [
                            ft.Container(
                                content=ft.Text(str(idx + 1), size=11, color="white",
                                ...
                                content=ft.Column(
                                    [
                                        ft.Text(desc, size=13, weight=ft.FontWeight.W_500,
                                                color=TEXT_PRIMARY, no_wrap=False),
                                        ft.Text(
                            ...
                            ft.Column(
                                [
                                    ft.Container(
                                        content=ft.Text(invoice_generator.format_idr(amt), size=12,
                    ...
                    margin=_m(bottom=8),
                    shadow=ft.BoxShadow(blur_radius=6, color="#00000020", offset=ft.Offset(0, 2))
                )
    ...
    def show_snack(msg: str, color: str = "#1e88e5"):
        icon_name = ft.Icons.CHECK_CIRCLE if color == SUCCESS else ft.Icons.ERROR_OUTLINE
    ...
    def add_item(e):
        ...
        items_list.append({
            ...
            "include_date": item_date_checkbox.value, "date": normalized_date
        })
        refresh_items_list()
        update_calculations()
    ...
    def gather_invoice_data():
        ...
        return {
            ...
            "items":           items_list,
            "tax_rate":        tax_rate,
            "tax_treatment":   "reverse" if tax_treatment_dropdown.value == "Reverse Charge" else "charge",
    ...
    def trigger_load_dialog(e):
        ...
        tax_treatment_dropdown.value = "Reverse Charge" if tax_code == "reverse" else "Standard Charge"
    ...
    def trigger_save_dialog(e):
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("Invoice Data Files", "*.json")]
        )
        if not path:
            return
    ...
    def trigger_preview(e):
        if not recipient_name_input.value.strip():
            ...
            return
        success, preview_path = invoice_generator.preview_in_browser(gather_invoice_data())
        if success:
        ...
        dlg = ft.AlertDialog(
            title=ft.Row(
                [ft.Icon(ft.Icons.PRINT, color=ACCENT),
                 ft.Text("Print/Save Invoice", size=16, weight=ft.FontWeight.BOLD, color=TEXT_PRIMARY)],
                spacing=8
            ),
            content=ft.Column(
                [
                    ft.Text("The HTML invoice has been opened in your browser.", color=TEXT_SECONDARY, size=13),
                    ft.Divider(color=DIVIDER, height=16),
                    ft.Text("On Android:", color=ACCENT, size=12, weight=ft.FontWeight.BOLD),
                    ft.Text(
            ...
            bgcolor=CARD_BG,
            actions=[
                ft.TextButton("Got it", style=ft.ButtonStyle(color=ACCENT), on_click=close_dlg)
            ],
    ...
    def trigger_pdf_export(e):
        ...
        if os.name == "nt":
            ...
            path = filedialog.asksaveasfilename(
                initialfile=suggested, defaultextension=".pdf",
            ...
            success, msg = invoice_generator.generate_pdf(gather_invoice_data(), "temp_render.pdf")
            page.launch_url(os.path.abspath(msg))
            show_print_help()

    # ── THEME TOGGLE ──
    theme_icon_btn = ft.IconButton(
        icon=ft.Icons.LIGHT_MODE, icon_color="white",
        icon_size=20, tooltip="Toggle theme"
    )
    ...
    view_profile = ft.ListView(
        controls=[
            ...
            accent_card(
                ft.Column(
                    [
                        section_header("YOUR COMPANY (CATERING)", ft.Icons.STORE, ACCENT),
                        comp_name_input,
                        ...
                        ft.Button(
                            content=ft.Row(
                                [ft.Icon(ft.Icons.SAVE_ALT, size=15, color="white"),
                                 ft.Text("Save as Default Profile", size=13, color="white")],
            ...
            accent_card(
                ft.Column(
                    [
                        section_header("BILL TO (CLIENT)", ft.Icons.PERSON_PIN, WARNING),
            ...
            accent_card(
                ft.Column(
                    [
                        section_header("INVOICE SETTINGS", ft.Icons.RECEIPT_LONG, ACCENT_LIGHT),
                        invoice_title_input,
                        ft.Container(height=6),
                        invoice_date_input,
                        ft.Container(height=6),
                        ...
                        ft.Container(
                            content=ft.Row(
                                [
                                    ft.Column(
                                        [
                                            ft.Text("Auto-Generate Number", size=13,
                                                    color=TEXT_PRIMARY, weight=ft.FontWeight.W_500),
                                            ft.Text("From client + date + counter", size=11, color=TEXT_SECONDARY)
                                        ],
                    ...
                    [
                        ft.Row(
                            [
                                ...
                                ft.Text("ADD NEW LINE ITEM", size=11, weight=ft.FontWeight.BOLD,
                                        color=ACCENT),
                        ...
                        ft.Button(
                            content=ft.Row(
                                [ft.Icon(ft.Icons.ADD_CIRCLE_OUTLINE, size=16, color="white"),
                                 ft.Text("Add Item to Invoice", size=13, color="white")],
                                tight=True, spacing=6
                            ),
                            on_click=add_item,
                            bgcolor=ACCENT,
            ...
            ft.Container(
                content=ft.Column(
                    [
                        ft.Row(
                            [
                                ft.Icon(ft.Icons.LIST_ALT, color=TEXT_SECONDARY, size=16),
                                ft.Text("CURRENT LINE ITEMS", size=11, weight=ft.FontWeight.BOLD,
    ...
    view_totals = ft.ListView(
        controls=[
            ...
            ft.Container(
                content=ft.Column(
                    [
                        section_header("CALCULATION SUMMARY", ft.Icons.CALCULATE, SUCCESS),
                        ft.Row(
                            [ft.Text("Subtotal", size=13, color=TEXT_SECONDARY, expand=True), subtotal_lbl],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN
                        ),
                        ...
                        ft.Container(height=1, bgcolor=DIVIDER, margin=_msym(vertical=12)),
                        ft.Row(
                            [
                                ft.Text("TOTAL DUE", size=14, weight=ft.FontWeight.BOLD, color=TEXT_PRIMARY),
                                total_due_lbl
                            ],
                            alignment=ft.MainAxisAlignment.SPACE_BETWEEN
                        )
                    ...
                    [
                        ...
                        ft.Row(
                            [
                                ft.Button(
                                    content=ft.Row(
                                        [ft.Icon(ft.Icons.FOLDER_OPEN, size=15, color=TEXT_PRIMARY),
                                         ft.Text("Open JSON", size=13, color=TEXT_PRIMARY)],
                                        tight=True, spacing=6
                                    ),
                                ...
                                ft.Button(
                                    content=ft.Row(
                                        [ft.Icon(ft.Icons.SAVE, size=15, color=TEXT_PRIMARY),
                                         ft.Text("Save JSON", size=13, color=TEXT_PRIMARY)],
                                        tight=True, spacing=6
                                    ),
            ...
            accent_card(
                ft.Column(
                    [
                        ...
                        ft.Button(
                            content=ft.Row(
                                [ft.Icon(ft.Icons.OPEN_IN_BROWSER, size=16, color="white"),
                                 ft.Text("Browser HTML Preview", size=13, color="white")],
                        ...
                        ft.Button(
                            content=ft.Row(
                                [ft.Icon(ft.Icons.PICTURE_AS_PDF, size=16, color="white"),
                                 ft.Text("Export A4 PDF Document", size=13, color="white",
                                         weight=ft.FontWeight.BOLD)],
                                tight=True, spacing=8
                            ),
        ...
        content=ft.Row(
            [
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Text("Catering Fajar", size=18, weight=ft.FontWeight.BOLD, color="white"),
